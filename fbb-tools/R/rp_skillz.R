...
DEFAULT_RP_SKILLZ_STABILIZATION_POINTS <- c(
  ...
  swstr_pct   = 75,    # pitches — stabilizes faster than CSW% (r=0.73 at 50 TBF vs CSW% ~300 TBF)
...
standardize_rp_skillz_input <- function(raw, lookback_season = NA_integer_) {
  ...
  p_throws    <- toupper(trimws(as.character(select_first_match(raw, c("Throws", "throws", "throw", "pitchhand", "pitch_hand", "hand")))))
  ...
  swstr_pct <- as_numeric_clean(select_first_match(raw, c("SwStr%", "swstr%", "swstr pct")))
