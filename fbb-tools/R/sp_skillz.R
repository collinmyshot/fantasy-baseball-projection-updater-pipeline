...
# ══════════════════════════════════════════════════════════════════════════════
# SP Skillz v2 — Ridge regression model (2021-2025 LOYO-CV)
# Metrics: 6 skill metrics, universal weights, no IP paradigm blending
# Target blend: K%=1/3, WHIP=1/3, SIERA=2/9, ERA=1/9
# ══════════════════════════════════════════════════════════════════════════════

SP_SKILLZ_V2_METRICS <- c(
  "k_minus_bb_pct",
  "whiff_pct",
  "stuff_plus",
  "pitching_plus",
...
SP_SKILLZ_V2_STABILIZATION_POINTS <- c(
  k_minus_bb_pct = 120,   # TBF-based
  whiff_pct      = 100,   # pitches-based
  stuff_plus     = 100,   # pitches-based
  pitching_plus  = 300,   # pitches-based
  ball_pct       = 150,   # pitches-based
  high_gb_flag   = 150    # TBF-based
)

SP_SKILLZ_V2_SAMPLE_SOURCE <- c(
  k_minus_bb_pct = "tbf",
  whiff_pct      = "pitches",
  stuff_plus     = "pitches",
...
# GB% threshold for high_gb_flag (FG returns GB% as 0-1 fraction)
SP_SKILLZ_V2_GB_THRESHOLD <- 0.55

SP_SKILLZ_V2_PLUS_METRICS <- c("stuff_plus", "pitching_plus")

# ══════════════════════════════════════════════════════════════════════════════
# SP Skillz v1 — DEPRECATED (kept for Streamonator and historical reference)
# ══════════════════════════════════════════════════════════════════════════════

SP_SKILLZ_METRICS <- c(   # v1 metric set (DEPRECATED — used by Streamonator)
  "tbf",
  "ip_per_gs",
  "siera",
  "xfip",
...
DEFAULT_SP_SKILLZ_WEIGHTS <- c(   # v1 weights (DEPRECATED)
  tbf = 0,
  ip_per_gs = 0,
  siera = -1,
  xfip = -1,
...
# Low-IP paradigm emphasizes metrics that held up earlier in-sample (v1 DEPRECATED).
...
standardize_sp_skillz_input <- function(raw, lookback_season = NA_integer_) {
  out <- data.frame(
    ...
    throws = toupper(trimws(select_first_match(raw, c("throws", "throw", "p_throws", "pitchhand", "pitcherhand", "hand")))),
    ...
    g  = as_numeric_clean(select_first_match(raw, c("g", "games", "gp", "app"))),
    ...
    swstr_pct = as_numeric_clean(select_first_match(raw, c("swstr%", "swstr pct", "sw str%"))),
  ...
  # GS/G fallback: when Start-IP / Relief-IP columns are absent from the API
  # response (e.g. custom FG type that omits them), use GS/G as a proxy.
  # A pitcher whose GS/G >= 2/3 is clearly a starter; one with GS == 0 is a
  # pure reliever. This mirrors the start_share_min threshold used downstream.
  needs_gs_fallback <- is.na(out$start_share) &
                       !is.na(out$gs) &
                       !is.na(out$g) &
                       out$g > 0
  if (any(needs_gs_fallback)) {
    out$start_share[needs_gs_fallback] <- out$gs[needs_gs_fallback] /
                                          out$g[needs_gs_fallback]
  }
...
fetch_and_score_sp_skillz <- function(
  ...
  scored <- compute_sp_skillz_scores(
    ...
    sp_depth = sp_depth,
    start_share_min = start_share_min,
    gs_qual_override = gs_qual_override,
    use_ip_paradigms = use_ip_paradigms,
    low_ip_max_start_ip = low_ip_max_start_ip,
    high_ip_min_start_ip = high_ip_min_start_ip,
    low_ip_weights = low_ip_weights,
...
# ══════════════════════════════════════════════════════════════════════════════
# SP Skillz v2 — Compute Function
# Ridge-derived universal weights, no IP paradigm blending, 6 skill metrics
# ══════════════════════════════════════════════════════════════════════════════

compute_sp_skillz_v2 <- function(
  skillz_data,
  weights           = SP_SKILLZ_V2_WEIGHTS,
  stab_points       = SP_SKILLZ_V2_STABILIZATION_POINTS,
  sample_sources    = SP_SKILLZ_V2_SAMPLE_SOURCE,
  plus_metrics      = SP_SKILLZ_V2_PLUS_METRICS,
  gb_threshold      = SP_SKILLZ_V2_GB_THRESHOLD,
  num_teams         = 15,
  sp_depth          = 10,
  start_share_min   = 2 / 3,
  reliability_method = "sample_over_sample_plus_stab"
  ...
  # Filter to starters
  keep <- !is.na(skillz_data$start_share) & skillz_data$start_share >= start_share_min
  out <- skillz_data[keep, , drop = FALSE]
  rownames(out) <- NULL
  if (nrow(out) == 0) stop("No rows remain after start-share filter.")

  # Compute high_gb_flag if not already present
  if (!"high_gb_flag" %in% names(out) && "gb_pct" %in% names(out)) {
    out$high_gb_flag <- as.integer(!is.na(out$gb_pct) & out$gb_pct >= gb_threshold)
  }
  ...
  # ── Pass 1: global z-scores for initial ranking ────────────────────────────
  z_global <- matrix(NA_real_, nrow = nrow(out), ncol = length(metrics))
  colnames(z_global) <- metrics
  for (m in metrics) {
    vals <- as.numeric(out[[m]])
    if (m %in% plus_metrics) {
      z_global[, m] <- (vals - 100) / 10
    } else {
      z_global[, m] <- z_score_vector(vals)
    }
  }

  # ── Build sample matrix + reliability ──────────────────────────────────────
  sample_mat <- matrix(NA_real_, nrow = nrow(out), ncol = length(metrics))
  colnames(sample_mat) <- metrics
  for (m in metrics) {
    src <- sample_sources[[m]]
    sample_mat[, m] <- pmax(0, suppressWarnings(as.numeric(out[[src]])), na.rm = FALSE)
  }

  rel_mat <- matrix(NA_real_, nrow = nrow(out), ncol = length(metrics))
  colnames(rel_mat) <- metrics
  for (m in metrics) {
    rel_mat[, m] <- compute_metric_reliability(
      samples = sample_mat[, m],
      stabilization_point = stab_points[[m]],
      method = reliability_method
    )
  }

  # Effective weights = weight * reliability
  w_vec <- as.numeric(weights[metrics])
  w_mat <- matrix(rep(w_vec, each = nrow(out)), nrow = nrow(out), ncol = length(metrics))
  colnames(w_mat) <- metrics
  eff_w_mat <- w_mat * rel_mat

  target_abs_weight <- sum(abs(weights))

  # Initial scores (global z)
  out$sp_skillz_score_initial <- vapply(seq_len(nrow(out)), function(i) {
    weighted_z_sum(z_global[i, ], w_mat[i, ], target_abs_weight)
  }, numeric(1))

  # ── Pass 2: starter-pool z-scores ──────────────────────────────────────────
  starter_n <- min(as.integer(round(num_teams * sp_depth)), nrow(out))
  if (starter_n < 1) stop("Starter pool size resolved to zero.")

  order_pass1 <- order(out$sp_skillz_score_initial, decreasing = TRUE, na.last = NA)
  starter_idx <- order_pass1[seq_len(min(starter_n, length(order_pass1)))]
  starter_pool <- out[starter_idx, , drop = FALSE]

  z_pool <- matrix(NA_real_, nrow = nrow(out), ncol = length(metrics))
  colnames(z_pool) <- metrics
  for (m in metrics) {
    vals <- as.numeric(out[[m]])
    if (m %in% plus_metrics) {
      z_pool[, m] <- (vals - 100) / 10
    } else {
      ref_vals <- as.numeric(starter_pool[[m]])
      z_pool[, m] <- z_score_from_reference(
        vals,
  ...
  # Final scores (pool-referenced z, reliability-weighted)
  out$sp_skillz_score <- vapply(seq_len(nrow(out)), function(i) {
    weighted_z_sum(z_pool[i, ], w_mat[i, ], target_abs_weight)
  }, numeric(1))
  out$sp_skillz_score_stabilized <- vapply(seq_len(nrow(out)), function(i) {
    weighted_z_sum(z_pool[i, ], eff_w_mat[i, ], target_abs_weight,
                   denominator_weights = w_mat[i, ])
  }, numeric(1))

  # Overall reliability
  row_abs_w     <- rowSums(abs(w_mat), na.rm = TRUE)
  row_abs_eff_w <- rowSums(abs(eff_w_mat), na.rm = TRUE)
  out$sp_skillz_reliability <- ifelse(row_abs_w > 0, row_abs_eff_w / row_abs_w, NA_real_)
  ...
  # Per-metric detail columns
  for (m in metrics) {
    out[[paste0(m, "_z")]]     <- z_pool[, m]
    out[[paste0(m, "_sample")]] <- sample_mat[, m]
    out[[paste0(m, "_rel")]]   <- rel_mat[, m]
    out[[paste0("w_", m)]]     <- w_mat[, m]
    out[[paste0("w_eff_", m)]] <- eff_w_mat[, m]
  }
