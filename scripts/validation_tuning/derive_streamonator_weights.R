...
#   3. ER  <= 3
#   4. WHIP <= 1.20
#   5. SP awarded the Win decision
#
# Optimization target: Spearman rank correlation between streamer_score
# and good_start_score (primary), plus mean good_start_score for the top 25%
# by streamer_score (secondary / interpretability).
#
# Inputs (fetched or loaded automatically):
#   - MLB Stats API boxscores: 2026 regular season SP lines
#   - FanGraphs API:           2026 SP Skillz (full season, qual=0)
#   - FanGraphs API:           2026 Team Rater (team batting, full season)
#   - Local CSV:               2026 park factors (overall_pf_idx_100)
#
# Outputs:
#   data/processed/streamonator_weight_analysis/starts_2026.csv
#   data/processed/streamonator_weight_analysis/weight_grid_2026.csv
#
# Run from project root:
#   Rscript scripts/validation_tuning/derive_streamonator_weights.R
...
PF_PATH    <- file.path("data", "processed", "park_factors",
                        "park_factors_savant_style_clean_2026_with_id.csv")
OUT_STARTS <- file.path(CACHE_DIR, "starts_2026.csv")
OUT_GRID   <- file.path(CACHE_DIR, "weight_grid_2026.csv")

dir.create(CACHE_DIR, showWarnings = FALSE, recursive = TRUE)

SEASON_START <- "2026-03-20"   # a few days before opening day to catch any early games
...
# ── Team ID → abbreviation (same map used by frosty-moore stream module) ───────
MLB_ID_TO_ABR <- c(
...
# Team abbreviation normalizer (consistent with Streamonator team_norm)
TEAM_MAP <- c(
  ARI="AZ",  ARZ="AZ",  AZ="AZ",
  ATH="ATH", OAK="ATH",
  BAL="BAL", BOS="BOS",
...
# ── 1. Fetch 2026 schedule → completed game PKs ────────────────────────────────
message("── Step 1: Fetching 2026 schedule ──────────────────────────────────────")
sched_cache <- file.path(CACHE_DIR, "schedule_2026.rds")
...
for (i in seq_len(nrow(game_index))) {
  meta    <- game_index[i, ]
  pk      <- meta$game_pk
  cache_f <- file.path(CACHE_DIR, sprintf("box_%d.rds", pk))
...
# K criterion: at least (floor(IP) - 1) strikeouts.
# "1 K per inning, with 1 allowed below" uses whole innings (floor) since
# partial innings don't count toward the rate threshold in common usage.
...
message("── Step 4: Fetching 2026 SP Skillz ─────────────────────────────────────")
spz_cache <- file.path(CACHE_DIR, "spz_2026_scores.rds")
spz_scores <- read_cache(spz_cache)

if (is.null(spz_scores)) {
  # Same leaderboard URL as pipeline.yml but with season=2026 and qual=0
  # (qual=0 ensures we capture pitchers early in the season with <5 IP)
  spz_url <- paste0(
    ...
    "&qual=0&season1=2026&season=2026"
  )
...
# Index to 100-centered scale (same as stream_index_spz in frosty-moore)
...
# Match starts → SP Skillz (exact normalized name, then last-name fallback)
...
starts$sp_skillz_index[starts$spz_placeholder] <- 100  # league-avg fill
...
# ── 5. Load + join Park Factor ─────────────────────────────────────────────────
message("── Step 5: Joining Park Factors ────────────────────────────────────────")
pf <- tryCatch(
  read.csv(PF_PATH, stringsAsFactors = FALSE, check.names = FALSE),
  error = function(e) stop("Park factor file not found: ", PF_PATH)
)
pf$overall_pf_idx_100 <- suppressWarnings(as.numeric(pf$overall_pf_idx_100))

# Park is where the game is played: the home team's ballpark
park_team <- ifelse(starts$home_away == "H", starts$pitcher_team, starts$opponent_team)
pf_match  <- match(park_team, pf$team_norm)

starts$park_factor_raw <- pf$overall_pf_idx_100[pf_match]
# Invert: higher PF = hitter-friendly = worse for pitcher (same as stream_build)
...
message("── Step 6: Fetching 2026 Team Rater ────────────────────────────────────")
tr_cache <- file.path(CACHE_DIR, "team_rater_2026.rds")
...
if (is.null(tr_df)) {
  tr_url <- paste0(
    ...
    "&season=2026&season1=2026&month=0",
  ...
  # Score: composite z-score (same logic as trater_score in mod_team_rater.R)
  # Directions: HR+, R+, BB%+, K%- (strikeouts are bad for opposing offense), wOBA+
...
# Join by opponent team (stronger opponent offense = harder start for pitcher)
tr_match <- match(starts$opponent_team, tr_df$team_norm)
starts$team_rater_raw <- tr_df$team_rater_index[tr_match]
# Invert: higher TR = tougher opponent = worse for pitcher (same as stream_build)
...
starts$team_rater_inv[starts$tr_placeholder] <- 100  # league-avg fill
...
# Simplex grid: w_sp + w_tr + w_pf = 1, each in {0, 0.1, …, 1.0}.
# Since stream_score() computes a weighted *mean*, only weight ratios matter —
# the simplex parameterization removes redundant duplicates without any loss.
# 66 unique combinations total.
...
grid_results <- do.call(rbind, lapply(seq_len(nrow(grid)), function(j) {
  ...
  # Weighted mean of components (mirrors stream_score logic)
...
# ── 8. Report ──────────────────────────────────────────────────────────────────
message("\n══════════════════════════════════════════════════════════════════")
message("  STREAMONATOR WEIGHT OPTIMIZATION — 2026 RESULTS")
message("══════════════════════════════════════════════════════════════════")
...
# Baseline (equal weights = 0.33:0.33:0.33)
